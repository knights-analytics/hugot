...
func crossEncoderPipeline(t *testing.T, session *Session) {
	...
	for i, expected := range results {
		if expected.Document != expectedResults[i].Document {
			t.Errorf("Expected document '%s', got '%s'", expectedResults[i].Document, expected.Document)
		}
		if math.Abs(float64(expected.Score-expectedResults[i].Score)) > 0.01 {
			t.Errorf("Expected score '%f', got '%f'", expectedResults[i].Score, expected.Score)
		}
	}
}
