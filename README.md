...
IMPORTANT: The Go backend is designed for simple tests, environments that disallow cgo, and for smaller models such as [all-MiniLM-L6-v2](https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2). It works best with small batches of 1-2 inputs per call. If you have performance requirements, please move to a C backend such as XLA or ORT (detailed below).
...
OpenXLA can be included at compile time via the build tag "-tags XLA". This typically provides up to a 100x speedup to heavy operations, and is more compatible with a wider range of models and tokenizers.

Onnx Runtime can also be selected as an alternative backend via the build tag "-tags ORT". It does not support training, but it is currently the fastest backend for inference.
