...
func loadRustTokenizer(tokenizerBytes []byte, pipeline *basePipeline) error {
	...
	// tokenizer init
	rustOptions, optErr := getRustTokenizerOptions(pipeline.InputsMeta)
	if optErr != nil {
		return optErr
	}
	pipeline.Tokenizer = &Tokenizer{Runtime: "RUST", RustTokenizer: &RustTokenizer{Tokenizer: tk, Options: rustOptions}, TokenizerTimings: &timings{}, Destroy: func() error {
		return tk.Close()
	}}
	return nil
}
